...
normal_fixed_mixture_stan_tau3 <-
    rstan::stan(model_code = readr::read_file("./bayesianideas_density_normal_fixed_mixture_tau_unordered.stan"),
                data = list(
                    ...
                    ## mu ~ N(m, s_squared)
                    m = rep(20,H), s_squared = rep(4,H),
                    ## Pi ~ Dirichlet(dirichlet_alpha / H)
                    dirichlet_alpha = rep(1,H) * H * 3,
                    ## Number of clusters
                    H = H,
                    n = nrow(galaxy),
                    y = galaxy$k_speed,
...
bad_chains <- c(6,8,9,10,14,19,28)
...
rstan::traceplot(normal_fixed_mixture_stan_tau3, pars = "lp__") +
    facet_wrap(~chain) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.key = element_blank(),
...
rstan::traceplot(normal_fixed_mixture_stan_tau3, pars = "mu[1]") +
    facet_wrap(~chain) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.key = element_blank(),
...
rstan::traceplot(normal_fixed_mixture_stan_tau3, pars = "mu[2]") +
    facet_wrap(~chain) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.key = element_blank(),
...
rstan::traceplot(normal_fixed_mixture_stan_tau3, pars = "mu[3]") +
    facet_wrap(~chain) +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.key = element_blank(),
...
bad_chains <- c(9L, 4L, 8L, 13L, 14L, 15L, 18L)
good_chains <- setdiff(seq_len(length(normal_fixed_mixture_stan_tau6@sim$samples)), bad_chains)

...
cunstruct_cumulative_hazard_function <- function(cutpoints, log_baseline_hazards, group_effect) {
    ...
    cumulative_hazard_function <- function(t, group) {
        ...
        if (length(t) > 1) {
            return(sapply(t, function(t) {
                cumulative_hazard_function(t, group)
            }))
        }

        ## We do not need intervals after t
        cutpoints_to_use <- cutpoints[cutpoints < t]
        ## diff(c(0)) -> numeric(0), which has length 0.
        ...
        return(baseline_cumulative_hazard * exp(group_effect * group))
    }
...
Now plot together for comparison.
...
survival_leukemia_together %>%
    df %>%
    ggplot(mapping = aes(y = survival_mean, group = interaction(treatment, model), color = model)) +
    geom_line() +

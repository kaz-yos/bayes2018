...
cut_times
## Every unit time
cut_unit <- seq(from = 1, to = cut_one, by = 1)
cut_unit
```
...
leukemia_times <- survival::survSplit(formula = Surv(time, status) ~ ., data = leukemia, cut = cut_times) %>%
    ...
    as_data_frame
leukemia_times
## Split at every unit time
leukemia_unit <- survival::survSplit(formula = Surv(time, status) ~ ., data = leukemia, cut = cut_unit) %>%
    mutate(interval = factor(tstart),
           interval_length = time - tstart) %>%
    as_data_frame
leukemia_unit
...
Some log baseline hazard estimates are -22.8454 with a standard error of 8693.5021. This occurs if the interval (a,b] does not contain any event, such as (13,16] in the model that defined intervals by all observed times.

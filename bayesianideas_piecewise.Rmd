...
Now we have posterior samples of interval-specific log baseline hazards as well as log hazard ratio for treatment. We can construct treatment group-specific posterior survival curves. Since we have piecewise constant hazards, it is easier to construct a cumulative hazard function. Here is the constructor.

```{r}
## This is easier than survival curve sampling.
cunstruct_cumulative_hazard_function <- function(cutpoints, log_baseline_hazards, group_effect) {
    function(t, group) {
        ## We do not need intervals after t
        cutpoints_to_use <- cutpoints[cutpoints < t]
        interval_lengths <- diff(c(cutpoints_to_use, t))
        n_zero_padding <- max(length(log_baseline_hazards) - length(interval_lengths), 0)
        zero_padding <- rep(0, n_zero_padding)
        interval_lengths_padded <- c(interval_lengths, zero_padding)
        baseline_cumulative_hazard <- sum(interval_lengths_padded * exp(log_baseline_hazards))
        baseline_cumulative_hazard * exp(group_effect * group)
    }
}
```

Here we construct posterior samples of cumulative hazard functions

```{r}
cum_haz_leukemia_one <- tidybayes::tidy_draws(fit_leukemia_one) %>%
    mutate(`H(t,group)` = pmap(list(`(Intercept)`, xNonmaintained),
    ...
    select(.chain, .iteration, .draw, `H(t,group)`)

cum_haz_leukemia_two <- tidybayes::tidy_draws(fit_leukemia_two) %>%
    mutate(`H(t,group)` = pmap(list(interval0, interval23, xNonmaintained),
...
cum_haz_leukemia_three <- tidybayes::tidy_draws(fit_leukemia_three) %>%
    mutate(`H(t,group)` = pmap(list(interval0, interval14, interval31, xNonmaintained),
...
cum_haz_leukemia_events <- tidybayes::tidy_draws(fit_leukemia_events) %>%
    mutate(`H(t,group)` = pmap(list(interval0, interval5, interval8, interval9, interval12,
...
cum_haz_leukemia_times <- tidybayes::tidy_draws(fit_leukemia_times) %>%
    mutate(`H(t,group)` = pmap(list(interval0, interval5, interval8, interval9, interval12, interval13,

...
```
### Define and compile prior check
```{r}
## Load and compile
piecewise_ph_prior_model <- rstan::stan_model("./bayesianideas_piecewise2_prior_only.stan")
piecewise_ph_prior_model
```
```{r,results="hide"}
piecewise_ph_prior_sample <-
    rstan::sampling(object = piecewise_ph_prior_model,
...
```{r}
piecewise_ph_prior_S_sample <- piecewise_ph_prior_sample %>%
    as.matrix(pars = "S_grid") %>%
    as_tibble()
names(piecewise_ph_prior_S_sample) <- as.character(grid)
piecewise_ph_prior_S_sample %>%

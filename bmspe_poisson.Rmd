...
##
plot_draws <- function(stan_fit, sample_indices) {
    extract_post_pred(stan_fit) %>%
...
plot_draws(stan_model_poisson, sample_indices = sample(seq_len(max(draw_data$.iteration)), size = 20))
```
...
plot_draws(stan_model_poisson_covs, sample_indices = sample(seq_len(max(draw_data$.iteration)), size = 20))
```
...
$$\begin{align*}
  &\text{Prior}\\
  \boldsymbol{\beta} &\sim MVN(\boldsymbol{0}, \mathbf{I})\\
  \gamma_{i} &\sim Gamma(a,b)\\
  \\
  &\text{Likelihood}\\
  \eta_{i} &= \mathbf{X}_{i}^{T}\boldsymbol{\beta}\\
  \mu_{i} &= e^{\eta_{i}}\\
  y_{i} &\overset{\text{ind}}{\sim} Poisson(\gamma_{i} \mu_{i})\\
  \end{align*}
$$
```{r}
...
plot_draws(stan_model_poisson_covs_gamma, sample_indices = sample(seq_len(max(draw_data$.iteration)), size = 20))
```

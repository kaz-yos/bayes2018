...
construct_gt <- function(thetahat_t) {
  gt <- function(theta0, theta1) {
    ...
    df <- tibble(X = c(5,9,8,4,7),
                 P_X_given_Z0 = dbinom(X, size = 10, prob = thetahat_t[1]),
                 P_X_given_Z1 = dbinom(X, size = 10, prob = thetahat_t[2]),
...
em_iter_out <- em_iter(c(0.6, 0.5))

thetahat0 <- as.numeric(em_iter_out[1,])
thetahat1 <- as.numeric(em_iter_out[2,])
thetahat2 <- as.numeric(em_iter_out[3,])
thetahat3 <- as.numeric(em_iter_out[4,])
thetahat4 <- as.numeric(em_iter_out[5,])
thetahat5 <- as.numeric(em_iter_out[6,])

g0 <- construct_gt(thetahat0)
g1 <- construct_gt(thetahat1)
g2 <- construct_gt(thetahat2)
g3 <- construct_gt(thetahat3)
g4 <- construct_gt(thetahat4)
g5 <- construct_gt(thetahat5)

lik_data_gt <- lik_data %>%
  mutate(exp_g0 = exp(g0(theta0, theta1)),
         exp_g1 = exp(g1(theta0, theta1)),
         exp_g2 = exp(g2(theta0, theta1)),
         exp_g3 = exp(g3(theta0, theta1)),
         exp_g4 = exp(g4(theta0, theta1)),
         exp_g5 = exp(g5(theta0, theta1)))

mat_lik <- lik_data_gt %>%
  select(theta0, theta1, lik) %>%
  spread(key = theta1, value = lik) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g0 <- lik_data_gt %>%
  select(theta0, theta1, exp_g0) %>%
  spread(key = theta1, value = exp_g0) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g1 <- lik_data_gt %>%
  select(theta0, theta1, exp_g1) %>%
  spread(key = theta1, value = exp_g1) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g2 <- lik_data_gt %>%
  select(theta0, theta1, exp_g2) %>%
  spread(key = theta1, value = exp_g2) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g3 <- lik_data_gt %>%
  select(theta0, theta1, exp_g3) %>%
  spread(key = theta1, value = exp_g3) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g4 <- lik_data_gt %>%
  select(theta0, theta1, exp_g4) %>%
  spread(key = theta1, value = exp_g4) %>%
  as.matrix(.) %>% `[`(,-1)
mat_exp_g5 <- lik_data_gt %>%
  select(theta0, theta1, exp_g5) %>%
  spread(key = theta1, value = exp_g5) %>%
  as.matrix(.) %>% `[`(,-1)
...
with(lik_data_gt,
     plot3d(x = theta0, y = theta1, z = lik,
            ...
            aspect = c(1, 1, 0.7)))
##
surface3d(grid, grid, mat_lik,    back = "line", front = "line", col = "gray", lwd = 1, alpha = 0.1)
points3d(x = thetahat0[1], y = thetahat0[2], z = exp(g0(thetahat0[1], thetahat0[2])))
rgl.snapshot(filename = "./source/em_rgl.png", fmt = "png", top = TRUE)
##
surface3d(grid, grid, mat_exp_g1, back = "line", front = "line", col = "blue",  lwd = 1, alpha = 0.1)
points3d(x = thetahat1[1], y = thetahat1[2], z = exp(g1(thetahat1[1], thetahat1[2])))
rgl.snapshot(filename = "./source/em_rgl1.png", fmt = "png", top = TRUE)
##
surface3d(grid, grid, mat_exp_g2, back = "line", front = "line", col = "blue", lwd = 1, alpha = 0.1)
points3d(x = thetahat2[1], y = thetahat2[2], z = exp(g2(thetahat2[1], thetahat2[2])))
rgl.snapshot(filename = "./source/em_rgl2.png", fmt = "png", top = TRUE)
##
surface3d(grid, grid, mat_exp_g3, back = "line", front = "line", col = "blue", lwd = 1, alpha = 0.1)
points3d(x = thetahat2[1], y = thetahat2[2], z = exp(g3(thetahat2[1], thetahat2[2])))
rgl.snapshot(filename = "./source/em_rgl3.png", fmt = "png", top = TRUE)
##
surface3d(grid, grid, mat_exp_g4, back = "line", front = "line", col = "blue", lwd = 1, alpha = 0.1)
points3d(x = thetahat4[1], y = thetahat4[2], z = exp(g4(thetahat4[1], thetahat4[2])))
rgl.snapshot(filename = "./source/em_rgl4.png", fmt = "png", top = TRUE)
##
surface3d(grid, grid, mat_exp_g5, back = "line", front = "line", col = "blue", lwd = 1, alpha = 0.1)
points3d(x = thetahat5[1], y = thetahat5[2], z = exp(g5(thetahat5[1], thetahat5[2])))
rgl.snapshot(filename = "./source/em_rgl5.png", fmt = "png", top = TRUE)
#+END_SRC

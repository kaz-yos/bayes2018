...
\begin{align*}
  ...
  &\propto \sum^{5}_{i=1}
    ...
    \right\}\\
  &= \sum^{5}_{i=1} (1-z_{i}) \left[ x_{i}\log\theta_{0} + (10-x_{i})\log(1-\theta_{0})\right]\\
  &~~~+\sum^{5}_{i=1} z_{i} \left[ x_{i}\log\theta_{1} + (10-x_{i})\log(1-\theta_{1})\right]
\end{align*}
...
*** @@latex:@@                                                        :BMCOL:
    ...
    :BEAMER_col: 0.50
    :END:
- A likelihood function is convex (one maximum).

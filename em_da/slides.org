...
- As with most optimization methods for nonconcave function (i.e., multiple local maxima), the EM algorithm comes with guarantees only of convergence to a local maximum.
...
*** @@latex:@@                                                        :BMCOL:
    ...
    :BEAMER_col: 0.50
    :END:
- Running the procedure using multiple initial starting parameters is often helpful.
- Similarly, initializing parameters in a way that breaks symmetry in models is also important.
- In this instance, at least three $\bthetahat$ exist: (0.80, 0.52), (0.52, 0.80), (0.66, 0.66).

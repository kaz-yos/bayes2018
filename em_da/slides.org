...
** Monotone Improvement in EM Algorithm
   ...
   :BEAMER_opt: allowframebreaks,label=,t
   :END:
- The EM Algorithm is guaranteed to improve the parameter estimates toward the local optimum.
\begin{align*}
...
\end{align*}
- This lower bound is maximized by reducing the KL divergence by setting $Q(\bz) = p(\bz | \bx, \btheta)$, which also gives equality.

...
for (i in seq_len(nrow(lik_data_gt) - 1)) {
  ...
  surface3d(x = grid, y = grid, z = lik_data_gt$mat[[i + 1]],
            ...
            lwd = 1, alpha = 0.1)
  ## Add next parameter estimate on gt surface
  points3d(x = gt_funs$theta0[i + 1], y = gt_funs$theta1[i + 1],
           z = exp(gt_funs$gt[[i]](gt_funs$theta0[i + 1], gt_funs$theta1[i + 1])))
  ...
  segments3d(x = c(gt_funs$theta0[i], gt_funs$theta0[i + 1]),
             ...
             z = )
  ## Save plot
  rgl.snapshot(filename = sprintf("./source/em_rgl_%s.png", lik_data_gt$gt[[i + 1]]),
               fmt = "png", top = TRUE)
}
#+END_SRC

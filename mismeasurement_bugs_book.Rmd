...
$$\begin{align*}
  p(\beta, \phi, \psi | Y, X_{mis}, X_{true})
...
$$\begin{align*}
  p(\beta, \phi, \psi | Y, X_{true})
  &\propto p(\beta, \phi, \psi, Y, X_{true})\\
  \\
  &= \sum^{1}_{x=0} p(\beta, \phi, \psi, Y, X_{mis}, X_{true} = x)\\
  \\
  &= \sum^{1}_{x=0} p(Y, X_{mis}, X_{true} = x | \beta, \phi, \psi) p(\beta, \phi, \psi)\\
  \\
  &= \sum^{1}_{x=0}
    p(Y | X_{mis}, X_{true} = x, \beta, \phi, \psi)
    p(X_{mis} | X_{true} = x, \beta, \phi, \psi)
    p(X_{true} = x | \beta, \phi, \psi)
    p(\beta, \phi, \psi)\\
  \\
  &~~~~\text{Using conditional independences in the DAG}\\
  \\
  &= \sum^{1}_{x=0}
    \underbrace{p(Y | X_{true} = x, \beta)}_{\text{outcome model}}
    \underbrace{p(X_{mis} | X_{true} = x,  \phi)}_{\text{error model}}
    \underbrace{p(X_{true} = x | \psi)}_{\text{covariate model}}
    \underbrace{p(\beta)p(\phi)p(\psi)}_{\text{priors}}\\
  \end{align*}
$$

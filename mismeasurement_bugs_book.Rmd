...
$$\begin{align*}
  ...
  &= p(\beta, \phi, \psi)\\
  &~~~\times \prod^{m}_{i=1}
    \underbrace{p(Y_{i} | X_{mis,i}, X_{true,i}^{r}, \beta)}_{\text{outcome model}}
    \underbrace{p(X_{mis,i} | X_{true,i}^{r}, \phi)}_{\text{error model}}
    \underbrace{p(X_{true,i}^{r} | \psi)}_{\text{covariate model}}\\
  &~~~\times \prod^{n}_{j=m+1}
    \underbrace{p(Y_{j} | X_{mis,j}, X_{true,j}^{c}, \beta)}_{\text{outcome model}}
...
$$\begin{align*}
  &\prod^{m}_{i=1}
  \sum^{1}_{x=0}
    p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=x, \beta)
    p(X_{mis,i} | X_{true,i}^{r}=x, \phi)
    p(X_{true,i}^{r}=x | \psi)\\
  &= \prod^{m}_{i=1}
    \left[
    p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=0, \beta)
    p(X_{mis,i} | X_{true,i}^{r}=0, \phi)
    p(X_{true,i}^{r}=0 | \psi) \right.\\
  &~~~~~~~~~~~\left. + p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=1, \beta)
...
$$\begin{align*}
  ...
  &~~~~~~~~~~~~~~\left. + p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=1, \beta)
    ...
    (\psi) \right]\\
  &~~~\times \prod^{n}_{j=m+1}
    p(Y_{j} | X_{mis,j}, X_{true,j}^{c}, \beta)
...
$$\begin{align*}
  ...
  &~~~~~~~~~~~~~~~~~\left. + p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=1, \beta)
    ...
    (\psi) \right]\\
  &~~~+ \sum^{n}_{j=m+1} \left[
    \log p(Y_{j} | X_{mis,j}, X_{true,j}^{c}, \beta)+
...
$$\begin{align*}
  ...
  &= \text{log_sum_exp}
    \left( \right.\\
  &~~~~~~\log p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=0, \beta) +
    \log p(X_{mis,i} | X_{true,i}^{r}=0, \phi) +
    \log (1-\psi),\\
  &~~~~~~\log p(Y_{i} | X_{mis,i}, X_{true,i}^{r}=1, \beta) +

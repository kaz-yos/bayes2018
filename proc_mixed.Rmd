...
```{r}
ar1_stan <- rstan::stan_model(file = "./proc_mixed_repeated_ar.stan")
ar1_stan
```
```{r, results="hide"}
ar1_stan_fit <-
    rstan::sampling(object = ar1_stan,
                    data = list(I = 48,
                                J = 9,
                                Mu_means = rep(0,9),
                                Mu_sds = rep(100,9),
...
```{r}
## Check HMC diagnostics after sampling
rstan::check_hmc_diagnostics(ar1_stan_fit)
## Specify relevant parameters
pars <- c("lp__","Mu","Sigma")
## Print a summary for a fitted model represented by a 'stanfit' object
print(ar1_stan_fit, pars = pars)
## Create a matrix of output plots from a 'stanfit' object
pairs(ar1_stan_fit, pars = c("Mu"))
pairs(ar1_stan_fit, pars = c("sigma"))
## Markov chain traceplots
rstan::traceplot(ar1_stan_fit, pars = c("Mu"), inc_warmup = FALSE)
rstan::traceplot(ar1_stan_fit, pars = c("sigma"), inc_warmup = FALSE)
## Trace plots of MCMC draws
regex_pars <- c("Mu")
bayesplot::mcmc_rank_hist(ar1_stan_fit, regex_pars = regex_pars, ref_line = TRUE)
bayesplot::mcmc_rank_overlay(ar1_stan_fit, regex_pars = regex_pars, ref_line = TRUE)
```

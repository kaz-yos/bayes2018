...
With degree = 0, we obtain a piecewise constant hazard function.

```{r}
stan_surv_mspline_degree0 <-
    stan_surv(formula = Surv(time, status) ~ x,
              data = leukemia,
              basehaz = "ms",
              basehaz_ops = list(degree = 0))
prior_summary(stan_surv_mspline_degree0)
summary(stan_surv_mspline_degree0)
```

```{r}
plot(stan_surv_mspline_degree0) + coord_cartesian(ylim = c(0, 0.1))
```
...
### Predicted survival function comparison

```{r}
stan_surv_exponential_surv <-
    posterior_survfit(stan_surv_exponential,
                      newdata = data.frame(x = c("Maintained",
                                                 "Nonmaintained")))
stan_surv_weibull_surv <-
    posterior_survfit(stan_surv_weibull,
                      newdata = data.frame(x = c("Maintained",
                                                 "Nonmaintained")))
stan_surv_mspline_surv <-
    posterior_survfit(stan_surv_mspline,
                      newdata = data.frame(x = c("Maintained",
                                                 "Nonmaintained")))
stan_surv_bspline_surv <-
    posterior_survfit(stan_surv_bspline,
                      newdata = data.frame(x = c("Maintained",
                                                 "Nonmaintained")))

(plot(stan_surv_exponential_surv) + labs(title = "Exponential")) /
    (plot(stan_surv_weibull_surv) + labs(title = "Weibull")) /
    (plot(stan_surv_mspline_surv) + labs(title = "M Spline")) /
    (plot(stan_surv_bspline_surv) + labs(title = "B Spline"))
...
The Stan code under the hood is the following.

```{r}
rstanarm:::stanmodels$surv
```

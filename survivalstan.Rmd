...
stan_weibull_survival_model_survival_functins <-
    ...
    mutate(`S(t|1)` = list(construct_survival_function(alpha, mu, beta, 1)),
           `S(t|0)` = list(construct_survival_function(alpha, mu, beta, 0)))
stan_weibull_survival_model_survival_functins
```
...
stan_weibull_survival_model_average_parameters <-
    ...
    summarize(alpha = mean(alpha),
              ...
              beta = mean(`beta_bg[1]`))
stan_weibull_survival_model_average_parameters
average_params_survival1 <- with(stan_weibull_survival_model_average_parameters,
                                 construct_survival_function(alpha, mu, beta, 1))
average_params_survival0 <- with(stan_weibull_survival_model_average_parameters,
                                 construct_survival_function(alpha, mu, beta, 0))

data_frame(t = seq(from = 0, to = 160, by = 0.1),
           `S(t|1)` = average_params_survival1(t),
           `S(t|0)` = average_params_survival0(t)) %>%
    gather(key = key, value = value, -t) %>%
    ggplot(mapping = aes(x = t, y = value, color = key, group = key)) +
    geom_line() +
    theme_bw() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
          legend.key = element_blank(),
